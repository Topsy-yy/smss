    ...
    <?php
      ...
      try{
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
          validate_csrf_or_redirect('signup_signatory_form', 'signup_sig.php');
          if (!empty($_POST["email"]) && !empty($_POST["password"])) {
            ...
            if($flag==0){
              ...
              // Write insert query
              $insertSignatory = $conn->prepare("INSERT INTO signatory(upMail,password) VALUES (?, ?)");
              $insertSignatory->bind_param("ss", $email, $phash);
              if ($insertSignatory->execute()) {
                $min = 100001;
                $max = 999999;
                ...
                if($insertVerify->execute()){
                  ...
                  $mail->Body    = $bodyContent;
                   ...
                   if(!$mail->send()) {
                       app_log('warning', 'Signatory verification email failed', array('email' => $email, 'error' => $mail->ErrorInfo));
                ...
                app_log('error', 'Signatory signup insert failed', array('email' => $email, 'error' => $conn->error));
      ...
      {
        app_log('error', 'Signatory signup exception', array('error' => $e->getMessage()));
      ...
      <div class = "col-xs-12 text-center">
        ...
        <div class="login">
          <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="POST" name="login" >
            <?php echo csrf_input('signup_signatory_form'); ?>
